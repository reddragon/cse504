...
%type <entity> ClassDeclaration ExtendsOpt
%type <entity>  ClassBodyDecl FieldDecl MethodDecl MethodHead ConstructorDecl
%type <entity> Variable FormalParam
%type <entity_list> ClassDeclarations ClassBodyDecls
%type <entity_list> Variables VarDecl VariablesCommaList
%type <entity_list> FormalsOpt FormalParamCommaList
...
%type <expression> Primary Literal FieldAccess ArrayAccess MethodInvocation
%type <expression> Expr DimExpr Assignment LeftHandSide ExprOpt
...
ClassDeclaration:
	TOK_CLASS TOK_ID ExtendsOpt
		...
		ClassBodyDecls 
		{
		   ((ClassEntity *)$<entity>4)->set_class_members($7);
		}
		TOK_CLOSE_BRACE
		{
...
ExtendsOpt:
	  TOK_EXTENDS TOK_ID {
	     bool flag;  	     
	     Entity* e = global_symtab->find_entity($2, CLASS_ENTITY, &flag);
	     if (e == NULL) {
	        cerr << "Superclass " << $2 << " not found." << endl;
		$$ = objectclass;
	     }   else
	       $$ = e;
	  }
...
Variable: TOK_ID DimStar 
          {
	     bool flag;
	     Entity *vd = global_symtab->find_entity($1, VARIABLE_ENTITY, &flag);
