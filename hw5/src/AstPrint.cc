...
// Print method for NativeStatement
void NativeStatement::print() {
  cout << "native \"" << this->function_name() << "\";" << endl;
}
...
void NewInstance::print() {
  cout << "new " << class_entity()->distinct_name() << "( ";
  list<Expression*>::iterator i;
  for(i=args()->begin(); i != args()->end();) {
    (*i)->print();
    i++;
